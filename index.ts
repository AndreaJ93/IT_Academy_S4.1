const chisteRandom: HTMLElement | null = document.getElementById("chiste");
const tiempoActual: HTMLElement | null = document.getElementById("tiempo");
let backgroundID: HTMLElement | null = document.getElementById("backgroundX");
let miniBackground: HTMLElement | null = document.getElementById("miniBackground");
let imagenTiempo: HTMLElement | null = document.getElementById("imagenTiempo");
const reportAcudits: Array<Object> = [];
let randomJoke: Boolean = true;
...
function visualizarChiste(chiste: Object | any) {
    if (chisteRandom) {
        chisteRandom.innerHTML = "";
    }
    const h3 = document.createElement("h3");
    h3.textContent = chiste.joke;
    chisteRandom?.appendChild(h3)
}

let puntuacion = "-";

function puntuacionChiste(id: string) {
    puntuacion = id;
}
...
function siguienteChiste(){
    ...
    reportAcudits.push({joke: chisteRandom?.textContent, score: puntuacion, date: isoDate});
    puntuacion = "-";
...
function visualizarTiempo(data: Object | any) {
    if (tiempoActual) {
...
function visualizarChiste2(chiste: Object | any) {
    if (chisteRandom) {
        chisteRandom.innerHTML = "";
    }
    const h3 = document.createElement("h3");
...
function stateSkyImage(stateSky: string){
